package com.kkpa.jbh.web.rest.users;
...
public class UsersGroupResource {
   ...
   * @return the ResponseEntity with status 201 (Created) and with body the new usersGroupDTO, or
   *         with status 400 (Bad Request) if the usersGroup has already an ID
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PostMapping("/users-groups")
  @Timed
  public ResponseEntity<UsersGroupDTO> createUsersGroup(@RequestBody UsersGroupDTO usersGroupDTO)
      throws URISyntaxException {
    log.debug("REST request to save UsersGroup : {}", usersGroupDTO);
    if (usersGroupDTO.getId() != null) {
      throw new BadRequestAlertException("A new usersGroup cannot already have an ID", ENTITY_NAME,
          "idexists");
    }
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the updated usersGroupDTO, or
   *         with status 400 (Bad Request) if the usersGroupDTO is not valid, or with status 500
   *         (Internal Server Error) if the usersGroupDTO couldn't be updated
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PutMapping("/users-groups")
  @Timed
  public ResponseEntity<UsersGroupDTO> updateUsersGroup(@RequestBody UsersGroupDTO usersGroupDTO)
      throws URISyntaxException {
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the usersGroupDTO, or with status
   *         404 (Not Found)
   */

...
import com.kkpa.jbh.web.rest.users.UsersGroupResource;
...
public class UsersGroupResourceIntTest {
  ...
  public void createUsersGroup() throws Exception {
    ...
    restUsersGroupMockMvc
        .perform(post("/api/users-groups").contentType(TestUtil.APPLICATION_JSON_UTF8)
  ...
  public void getAllUsersGroups() throws Exception {
    ...
    restUsersGroupMockMvc.perform(get("/api/users-groups?sort=id,desc")).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(usersGroup.getId().intValue())))
        .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME.toString())))
        .andExpect(jsonPath("$.[*].invitationAccepted")
            .value(hasItem(DEFAULT_INVITATION_ACCEPTED.booleanValue())));
  }
  ...
  public void getUsersGroup() throws Exception {
    ...
    restUsersGroupMockMvc.perform(get("/api/users-groups/{id}", usersGroup.getId()))
        ...
        .andExpect(jsonPath("$.name").value(DEFAULT_NAME.toString())).andExpect(
            jsonPath("$.invitationAccepted").value(DEFAULT_INVITATION_ACCEPTED.booleanValue()));
  }
  ...
  public void updateUsersGroup() throws Exception {
    ...
    updatedUsersGroup.name(UPDATED_NAME).invitationAccepted(UPDATED_INVITATION_ACCEPTED);
    UsersGroupDTO usersGroupDTO = usersGroupMapper.toDto(updatedUsersGroup);

    restUsersGroupMockMvc
        .perform(put("/api/users-groups").contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(usersGroupDTO)))
        .andExpect(status().isOk());
  ...
  public void deleteUsersGroup() throws Exception {
    ...
    // Get the usersGroup
    restUsersGroupMockMvc.perform(
        delete("/api/users-groups/{id}", usersGroup.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
        .andExpect(status().isOk());
